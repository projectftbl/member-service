...
Connector.prototype.connect = function(member, connection) {
  ...
  return Connection.find(data).then(function(connections) {
    ...
    return Connection.create(data).then(function(connection) {
      ...
      sceneskope('member:connection:create', data, context);
...
Connector.prototype.connections = function(member, type, options) {
  return Connection.listByMemberForType(member, type, options).then(function(connections) {
    return Connection.listByConnectedToForType(member, type, options).then(function(connectedTo) {
      return populate(connections.concat(connectedTo));
    });
  });
};
