...
Repository.prototype.listByStatsId = function(id) {
  return this.find({ statsId: parseInt(id, 10) });
};

Repository.prototype.listByType = function(type) {
...
Repository.prototype.sanitize = function(member) {
  ...
  if (member.additional) {
    var data = member.additional;
    if (data.equity && !isNaN(data.equity)) member.additional.equity = parseFloat(data.equity); 
    if (data.term && !isNaN(data.term)) member.additional.term = parseFloat(data.term); 
    if (data.budget && !isNaN(data.budget)) member.additional.budget = parseFloat(data.budget); 
  }
